...
double angleBetweenLines(CvPoint start_line_1, CvPoint end_line_1, CvPoint start_line_2, CvPoint end_line_2)
{
	...
	double a_l = sqrt(x1*x1+y1*y1);
	double b_l = sqrt(x2*x2+y2*y2);
...
double getDiffPoints(CvPoint const &p1, CvPoint const &p2)
{
	...
	double dist = static_cast<double>((pow((double)diff_x, 2) + (double)pow((double)diff_y, 2)));
	dist = sqrt(dist);
	return dist;
}
...
int LaneFinder::publishCurve( std::vector< CvPoint >  &curve)
{
    ...
    /*
    n = write(TCPBUS_SOCKFD,busprefix,strlen(busprefix));
    if (n < 0){
...
//	    file_out<<curve[i].x<<"\t"<<curve[i].y<<endl;
        n = write(TCPBUS_SOCKFD,buffer,strlen(buffer));
        if (n < 0){
            cout << "ERROR: Error writing to the bus socket\n";
            return(-1);
        }
...
void LaneFinder::findLines(IplImage *img, IplImage *canny_img, double* alpha,int &x_pos, std::vector< struct line_points*> &longestLine, int lowerLimit, int higherLimit, int yLimit,  std::vector< std::vector< struct line_points* > > &bin, std::vector< std::vector< struct line_points* > > &binRight, int minSize, int lengthTangent)
{
	...
	for(int i=0; i<n;i++){
		...
		if(bin[i].size() >0){
			if(a> sizeTmp && (bin[i][0]->point.x<=higherLimit) && (bin[i][0]->point.x>=lowerLimit) &&  bin[i][0]->point.y > yLimit){
                sizeTmp=a;
                helper=i;
			}
        }
	...
	for(int i=0; i<n;i++){
		...
		if(a > sizeTmp && (binRight[i][0]->point.x<=higherLimit) && (binRight[i][0]->point.x>=lowerLimit) &&  binRight[i][0]->point.y > yLimit)
		{
			sizeTmp=a;
			helperRight=i;
		}
...
void LaneFinder::getTangentParams(std::vector< struct line_points*> &longestLine,  int &highest_y_index, double &m, double &b, double &angle, CvPoint &startpoint, CvPoint &endpoint, int length, int img_width, IplImage *img)
{
		  ...
		  angle=atan2((double)(getHeight()-endpoint.y)- (getHeight()-startpoint.y), endpoint.x-startpoint.x);
					 ...
					 for(int i=length/2; i<length; i+=windowSize/2)//not the entire image, only part of it
					 {
								...
								if(i+windowSize<length)
								{
										  ...
										  for(int a=0; a<=windowSize;a++)
										  {
													 ...
													 p.x=static_cast<int>(round(longestLine[i]->point.x+cosM*c));
													 p.y=static_cast<int>(round(height-(height-(longestLine[i]->point.y)+sinM*c)));
...
int LaneFinder::extractLine(IplImage *imgInput, IplImage *imgOutput, std::vector<CvPoint> &lineRight,  int lowerLimit, int higherLimit, bool debug, int minSize, int lengthTangent)
{
	  ...
	  int yLimit=imgInput->height/2;
	  findLines(imgInput, img_out, _alpha, x_pos_tmp, longestLine, lowerLimit, higherLimit, yLimit, bin, binRight, minSize, lengthTangent);
	  if(debug)
	  {
